...
namespace Roadrunner
{
    public partial class Scanning : Form
    {
        ...
        private void Scanning_Load(object sender, EventArgs e)
        {
            wo.Id_wo = 2;
            user.Id_user = 1;
            dg_Scan.DataSource = scan.LlenarDG("select sc.sn as 'Serial Number', sc.dateReg as 'Date Scan' , pn.pn as 'Part Number' from tb_Scan sc join tb_WO wo on sc.id_wo = wo.id_wo join tb_PN pn on pn.id_pn = wo.id_pn join tb_Box box on box.id_box = sc.box where wo.id_wo = '" + wo.Id_wo + "'").Tables[0];
        ...
        private void btn_Submit_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(txt_Scan.Text.Trim()))
            {
                ...
                //if (CheckTest() == true)
                //if (scan.Existe("select count(*) from tb_test where SN = '" + txt_Scan.Text.Substring(txt_Scan.Text.ToString().Trim().Length - 4) + "'"))
                if (scan.Existe("select count(*) from tb_test where SN = '" + txt_Scan.Text.Trim() + "'"))
                {
                    //GetSN();
                    //if (!scan.Existe("select count(*) from tb_Scan where sn = '" + txt_Scan.Text.Substring(txt_Scan.Text.ToString().Trim().Length - 4) + "'"))
                    if (!scan.Existe("select count(*) from tb_Scan where sn = '" + txt_Scan.Text.Trim() + "'"))
                    {

                        scan.Crud("insert into tb_Scan(sn, dateReg, id_wo, id_user, box) values('" + txt_Scan.Text.Trim() + "','" + DateTime.Now + "','" + wo.Id_wo + "','" + user.Id_user + "','" + scan.id_box + "')");
                        BoxControl();
                        txt_Scan.Text = "";
                ...
                {
                    MessageBox.Show("Unit Fail Test!");
                }
            }
            else
        ...
        private bool CheckTest()
        {
            ...
            test.result = test.ReturnValue("select result from test where cp_sin = '" + txt_Scan.Text.Trim() + "'");
